...
void run_test(s_resval *res, int nrepeat, NFFT_INT m, int nfsft_flags, int psi_flags, int nthreads)
{
  ...
  if (nthreads < 2)
    snprintf(cmd, 1024, "%s %td %d %d %d < nfsft_benchomp_test.data > nfsft_benchomp_test.out", CMD_DETAIL_SINGLE, m, nfsft_flags, psi_flags, nrepeat);
  else
    snprintf(cmd, 1024, "%s %td %d %d %d %d < nfsft_benchomp_test.data > nfsft_benchomp_test.out", CMD_DETAIL_THREADS, m, nfsft_flags, psi_flags, nrepeat, nthreads);
  fprintf(stderr, "%s\n", cmd);
...
void print_output_speedup_total_tref(FILE *out, s_testset *testsets, int ntestsets, int use_tref, double tref)
{
  ...
  for (t = 0; t < ntestsets; t++)
  {
    ...
    fprintf(stderr, "%s $\\mathrm{NFSFT}%s$ N=%td M=%td m=%td %s %s %s}", hostname, testset.param.trafo_adjoint==0?"":"^\\top", testset.param.N, testset.param.M, testset.param.m, get_psi_string(testset.param.psi_flags), get_sort_string(testset.param.psi_flags), get_adjoint_omp_string(testset.param.psi_flags));
    fprintf(stderr, "\n");
...
void print_output_histo_PENRT(FILE *out, s_testset testset)
{
  ...
  fprintf(out, " title={%s $\\mathrm{NFSFT}%s$ N=%td M=%td m=%td %s %s %s}", hostname, testset.param.trafo_adjoint==0?"":"^\\top", testset.param.N, testset.param.M, testset.param.m, get_psi_string(testset.param.psi_flags), get_sort_string(testset.param.psi_flags), get_adjoint_omp_string(testset.param.psi_flags));
  fprintf(out, " ]\n");
  fprintf(out, "\\addplot coordinates {");
  for (i = 0; i < size; i++)
